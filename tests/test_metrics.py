...
class TestFusionGain:
    ...
    def test_zero_gain_when_equal(self):
        g = fusion_gain(0.5, 0.5, 0.3)
        assert g == 0.0

    def test_zero_fused_prob_returns_zero(self):
        g = fusion_gain(0.0, 0.5, 0.3)
        assert g == 0.0

    def test_best_parent_zero_returns_zero(self):
        g = fusion_gain(0.5, 0.0, 0.0)
        assert g == 0.0

    def test_fusion_worse_than_either(self):
...
class TestParentProbForToken:
    def test_finds_prob_in_top_k(self):
